...
<script src = "https://code.jquery.com/ui/1.10.4/jquery-ui.js"></script>
    ...
    <script
    src='viewProfileScoutingPopup.js'>
    </script>     
        
    <!--link to call jquery file to cancel editing positions-->
    <script
    src='viewProfileCancelProfileScoutingPopup.js'>
    </script> 
...
$findScoutingFirstname = $pdo->prepare('SELECT firstname FROM members WHERE username=?'); 

//execute query with variables
$findScoutingFirstname->execute([$userProfile]);
        
($scoutingFirstname = $findScoutingFirstname->fetchColumn());
    
    
//find users surname
$findScoutingSurname = $pdo->prepare('SELECT surname FROM members WHERE username=?'); 

//execute query with variables
$findScoutingSurname->execute([$userProfile]);
        
($scoutingSurname = $findScoutingSurname->fetchColumn());
    
    
 $scoutingFullname = $scoutingFirstname.' '.$scoutingSurname;
...
$scoutUser->execute([$userProfile, $scoutingFullname, $username, $fullname]);
...
{     
        ...
        //execute query and variables
$unscoutUser->execute([$userProfile, $username]);
...
//execute query with variables
$scoutingAlready->execute([$userProfile, $username]);
...
echo <<<_END
 ...
 <form action="viewProfileScouting.php" method="post" id="scoutForm">
...
<form action="viewProfileScouting.php" method="post" id="unscoutForm">
...
//execute query with variables
$scoutingNumber->execute([$userProfile]);
...
//execute query with variables
$scoutedNumber->execute([$userProfile]);
...
print '<form action="viewProfileScouting.php" id="profileScoutingForm" method="post">';
...
//execute query with variables
$scoutingList->execute([$userProfile]);
...
print '<form action="viewProfileScouting.php" id="profileScoutedForm" method="post">';
...
//execute query with variables
$scoutedList->execute([$userProfile]);
